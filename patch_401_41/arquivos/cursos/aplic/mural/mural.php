...
/* Define se existe mensagem no mural ou ainda está vazio.*/
  ...
  /* Se usuário não é convidado passivo, ou o curso ainda está ativo ou ele é      */
...
/* Se o curso estiver encerrado e for um aluno, n�o pode inserir mensagens */
  /* Convidados passivos e visitantes n�o podem postar mensagens             */
  if (!EConvidadoPassivo($sock, $cod_usuario, $cod_curso)) {
  ...
  if($existe_mensagem){
    ...
    echo("                <select class=\"text\" name=\"ordem\" id=\"ordem_msg\" onchange='MudaOrdenacao();'>\n");
    /* 5 - data */
...
// /*var_dump*/($lista_mensagens);
    foreach($lista_mensagens as $cod_msg => $dados){
      ...
      /* Retorna o nome do usuario correspodente ao cod_usuario */
        ...
        echo("                          <div class=\"divRichText\" style=\"width:595px;overflow:auto;border:1px solid;\";>". PreparaExibicaoMensagem($dados['texto'])."</div>\n");
        echo("                        </td>\n");

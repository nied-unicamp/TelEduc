...
/***************************************************************************************************************************/
    ...
    echo("        expressao=expressao.replace(new RegExp(/COS\(/g), 'Math.cos(');\n");
    echo("        expressao=expressao.replace(new RegExp(/EXP\(/g), 'Math.exp(');\n");
    echo("        expressao=expressao.replace(new RegExp(/CH�O\(/g), 'Math.floor(');\n");
    echo("        expressao=expressao.replace(new RegExp(/CHAO\(/g), 'Math.floor(');\n");
    echo("        expressao=expressao.replace(new RegExp(/LOG\(/g), 'Math.log(');\n");
    echo("        expressao=expressao.replace(new RegExp(/M�XIMO\(/g), 'Math.max(');\n");
    echo("        expressao=expressao.replace(new RegExp(/MAXIMO\(/g), 'Math.max(');\n");
    echo("        expressao=expressao.replace(new RegExp(/M�NIMO\(/g), 'Math.min(');\n");
    ...
    echo("          if (i == 0) {\n");
    if ($usr_formador) {
    // 191 - Verifique se a sua express�o est� correta!
      //echo("        alert('Verifique se sua express�o est� correta!\\nErro = ' + e.message);\n");
    ...
    echo("            ok=false;\n");
    // ?? - Verifique se a norma est� correta!
    ...
    // 198 - A divis�o por 0 (zero) n�o � permitida!
    ...
    // Ver objetivos e crit�rios da avaliacao
  ...
  echo("                  <li><a href=\"#\" title=\"Avalia&ccedil;&otilde;es passadas\" onClick=return(VerTelaAvaliacoes('P'))>".RetornaFraseDaLista($lista_frases, 29)."</a></li>\n");
  /* 32 - Avalia��es Atuais*/
  echo("                  <li><a href=\"#\" title=\"Avalia&ccedil;&otilde;es atuais\" onClick=return(VerTelaAvaliacoes('A'))>".RetornaFraseDaLista($lista_frases, 32)."</a></li>\n");
  /* 30 - Avalia��es Futuras*/
  echo("	          <li><a href=\"#\" title=\"Avalia&ccedil;&otilde;es futuras\" onClick=return(VerTelaAvaliacoes('F'))>".RetornaFraseDaLista($lista_frases, 30)."</a></li>\n");
  /* 31 - Notas dos participantes */
...
$sock = MudarDB($sock, $cod_curso);
    ...
    /* Notas dos Formadores (Só são vistas por formadores) */
    if ($usr_formador)
      {
        /* 156 - Formador */
        echo("                  <tr class=\"head\">\n");
  ...
  echo("                  <li><span id=\"span_legenda\" onClick=\"ExibirLegenda();\">Exibir Legenda</span></li>");
  echo("                  <li><span onClick=\"SalvarTodasNotas();\">Salvar em arquivo</span></li>\n");
  echo("                  <li><span onClick=\"ImprimirRelatorio();\">Imprimir</span></li>\n");
  echo("                </ul>\n");
  echo("              </td>\n");
  echo("            </tr>\n");
  echo("          </table>\n");
  ...
  if ($apresentar_legenda)
  {
    ...
    /*?? -  Período */
    echo("              <td bgcolor=\"#f1f1f1\"><b>Per&iacute;odo</b></td>\n");
  ...
  if (($usr_formador) && (!isset($SalvarEmArquivo)))
  {
    ...
    echo("                  <input type=hidden name=tela_avaliacao value='".$tela_avaliacao."' />\n"); 
    /* 18 - Ok (gen) */
    echo("                  <input type=\"submit\" id=\"ok_novaexpressao\" class=\"input\" value=\"".RetornaFraseDaLista($lista_frases_geral,18)."\" />\n");
    /* 2 - Cancelar (gen) */

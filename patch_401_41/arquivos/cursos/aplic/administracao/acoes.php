...
//   if($action_ger == "ApagaUsuario")
//   {
//     $usuarios = explode(",",$elementos);
//     foreach ($usuarios as $cod_usu)
//     {
//       MudaTipoUsuario($sock,$cod_curso,$cod_usu,'X');
//     }
//     header("Location:".$origem."?cod_curso=".$cod_curso."&cod_usuario=".$cod_usuario."&cod_ferramenta=".$cod_ferramenta."&acao=".$acao);
// 
//   }
  ...
  {
    echo("    <form name=\"formAcoes\" method=\"get\" action=\"inscrever3.php\">\n");
...
$codigos_usu_global = explode(",", $codigos_usu_global_aux);
  ...
  if($action == "inscrever")
  {
    ...
    echo("    <form name=\"formAcoes\" method=\"get\" action=\"inscrever.php\">\n");
    echo("      <input type=\"hidden\" name=\"cod_curso\" value=\"".$cod_curso."\">\n");
    echo("      <input type=\"hidden\" name=\"cod_usuario\" value=\"".$cod_usuario."\">\n");
    echo("      <input type=\"hidden\" name=\"cod_ferramenta\" value=\"0\">\n");
    echo("      <input type=\"hidden\" name=\"tipo_usuario\" value=\"".$tipo_usuario."\">\n");
    echo("      <input type=\"hidden\" name=\"acao_fb\" value=\"dadosPreenchidos\">\n");
    echo("      <input type=\"hidden\" name=\"atualizacao\" value=\"false\">\n");
    ...
    foreach($dados_preenchidos_s as $cod => $linha)
    {
      ...
      echo("      <input type=\"hidden\" name=\"dados_preenchidos_s[]\" value=\"".$dados_preenchidos_s[$cod]['nome']."\">\n");
      echo("      <input type=\"hidden\" name=\"dados_preenchidos_s[]\" value=\"".$dados_preenchidos_s[$cod]['email']."\">\n");
      echo("      <input type=\"hidden\" name=\"dados_preenchidos_s[]\" value=\"".$dados_preenchidos_s[$cod]['login']."\">\n");
    }
    ...
    if(($login_existente == true) || ($email_existente == true))
    {
      echo("    </form>\n");
      echo("    <script type=\"text/javascript\">\n");
      echo("      document.formAcoes.submit();\n");
      echo("    </script>\n");
    }
    else
    {
...
//   if($action == "alterarNomenclatura")
//   {
//     if(!AlteraCursoConfig($sock, 'status_coordenador', $status))
//     {
//       /*? - */
//       $msgErro = "Erro ao alterar nomenclatura!!";
//     }  
//     else
//     {
//       Desconectar($sock);
//       header("Location:administracao.php?cod_curso=".$cod_curso."&cod_usuario=".$cod_usuario."&cod_ferramenta=0");
//     }
//   }

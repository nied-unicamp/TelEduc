...
-------------------------------------------------------------------------------
    ...
    Arquivo : cursos/aplic/mural/excluir_pergunta.php
...
/*==========================================================
  ...
  /* 42 - Excluir Pergunta */
  $cabecalho .= "  <b class=subtitulo> - ".RetornaFraseDaLista($lista_frases, 42)."</b>";
  ...
  if(!isset($check))
    $verif=ExcluiPergunta($sock, $cod_pergunta);
  else
    $verif=ExcluiPergunta($sock, $check);
  if ($verif)
  {
    /* 44 - Pergunta excluída com sucesso. */
    echo("    <font class=text>".RetornaFraseDaLista($lista_frases, 44)."</font>\n");
    echo("    <p>\n");
    /* 23 - Voltar */
    echo("    <input type=submit class=text value='".RetornaFraseDaLista($lista_frases_geral, 23)."'>\n");
    echo("  </form>\n");
  }
  else /* Se a pergunta NAO pode ser excluida apresenta uma mensagem de erro */
  {
    /* 45 - Erro ao excluir a pergunta. */
    echo("    <font class=text>".RetornaFraseDaLista($lista_frases,  45)."</font>\n");
    echo("    <p>\n");

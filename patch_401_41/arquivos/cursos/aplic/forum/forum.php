...
//   echo("      var msg_57='".RetornaFraseDaLista($lista_frases, 57)."';\n");
  ...
  if (($status == 'A')&&($AcessoAvaliacao))
  {
    ...
    echo("         window.open(\"../avaliacoes/ver.php?".RetornaSessionID()."&cod_curso=".$cod_curso."&VeioDaAtividade=1&EhAluno=".$usr_aluno."&cod_avaliacao=\"+id,\"VerAvaliacao\",\"width=600,height=400,top=120,left=120,scrollbars=yes,status=yes,toolbar=no,menubar=no,resizable=yes\");\n");
    echo("        return(false);\n");
    echo("      }\n\n");
  ...
  echo("                <select name=\"ordem_foruns\" id=\"ordem_foruns\" onchange='MudaOrdenacao();' style=\"margin:5px 0 0 0; \">\n");
  /* 44 - data */
  ...
  if (($total_foruns)==0) {
    ...
    foreach($lista_foruns as $num => $linha_lista_foruns)
    {
      ...
      */
      ...
      if($AcessoAvaliacao){
        if (ForumEhAvaliacao($sock,$lista_foruns[$num]['cod_forum'])){

...
/*==========================================================
  ...
  ExibirCabecalho(4,"Cria��o do arquivo teleduc.conf");
  ...
  if ($sock==-1)
    Voltar("N�o foi poss�vel conectar ao MySQL. Pode ser que o MySQL n�o esteja corretamente instalado, ou que o Deamon do MySQL n�o esteja em execu��o, ou o login e senha de root tenham sido digitados errados.<br><br>Mensagem de erro retornada pelo MySQL: <font color=black>".mysql_error()."</font>");

  $tmpdbbasecurso_s=$tmpdbbasecurso;
  $tmpteleduc_login_s=$tmpteleduc_login;
  $tmpteleduc_senha_s=$tmpteleduc_senha;

#create temporary tables ,lock tables, reload e file foram retirados para manter compatibilidade com vers�es anteriores � 4.0.2 do MySQL
...
#Estes inserts fazem a mesma coisa que o grant acima, n�o s�o necess�rios.
/*
  if ($versao_numero == 3)
...
#Caso seja pedido uma base tempor�ria (passo 3 da instala��o) seta permiss�es para um usu�rio criar tais bases, 
#Usadas durante a importa��o.
  ...
  if ($importar_curso_extr_s)
  {
    ...
    /* TODO - Se for usu�rios diferentes, deve-se dar permiss�o ao tmp para o banco geral e dos cursos. */
    if($tmpteleduc_login_s!=$teleduc_login_s) /*temos q inserir na tabela usu�rio tb.*/
    {
...
#Query desness�ria pois seta os mesmos privil�gios que o grant logo no in�cio, estava repetido.
/*
...
");
  ...
  Paragrafo("1 - Usu�rios inseridos com sucesso!");
  echo("<br/>\n");
  ...
  Paragrafo("<b><font size=+1>Preencha abaixo os diret�rios e caminhos de execu��o para uso interno do ambiente</font></b>");
  CaixaTexto("Diret�rio que cont�m os arquivos do TelEduc (deve ser acess�vel pela web. <b>Ex.:</b> /home/teleduc/public_html):","dirtele", $dirtele);
  CaixaTexto("Diret�rio no qual ficar� o arquivo de configura��o do TelEduc, \"<b><i>teleduc.conf</i></b>\" (deve ficar fora do acesso da web. <b>Ex:</b> /home/teleduc):","ambiente", $ambiente);
  CaixaTexto("Diret�rio em que ficar�o os arquivos anexados a itens das ferramentas do curso (deve ficar fora do acesso da web. <b>Ex:</b> /home/teleduc/arquivos):","arquivos",$arquivos); 
  CaixaTexto("Diret�rio em que ser� criados links para exibi��o dos arquivos pela web (<b>Ex:</b> /home/teleduc/public_html/diretorio):","arquivosweb",$arquivosweb); 
  CaixaTexto("Diret�rio para o qual ser�o movidos os cursos extraidos (<b>Ex:</b> /home/teleduc/extraidos):","extracao",$extracao); 
   
  if($importar_curso_extr_s) 
    CaixaTexto("Diret�rio onde ser�o montados os cursos extra�dos durante a importa��o (<b>Ex:</b> /home/teleduc/montagem):","montagem",$montagem);
   
  CaixaTexto("Caminho via browser (sem o nome da m�quina. <b>Ex:</b> /~teleduc):","raiz_www",$raiz_www); 

  if (file_exists($sendmail))
    CaixaTexto("Caminho do Sendmail (inclusive o execut�vel):","sendmail",$sendmail);
  else
    CaixaTexto("Caminho do Sendmail (inclusive o execut�vel):","sendmail","", "&nbsp;Ex.: ".$sendmail);

  if (file_exists($mysqldump))
    CaixaTexto("Caminho do mysqldump (inclusive o execut�vel):","mysqldump",$mysqldump);
  else
    CaixaTexto("Caminho do mysqldump (inclusive o execut�vel):","mysqldump","","&nbsp;Ex.: ".$mysqldump);

  if (file_exists($mimetypes))
    CaixaTexto("Caminho do arquivo \"mime.types\" do Apache para resolu��o do arquivos (inclusive o nome do arquivo):","mimetypes",$mimetypes);
  else
  ...
  if (file_exists($tar))
    CaixaTexto("Caminho do tar (inclusive o execut�vel):","tar",$tar);
  else
    CaixaTexto("Caminho do tar (inclusive o execut�vel):","tar","", "&nbsp;Ex.: ".$tar);

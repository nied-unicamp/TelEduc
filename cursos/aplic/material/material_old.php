...
/*==========================================================
  ...
  /* Registrando cï¿½igo da ferramenta nas variï¿½eis de sessï¿½.
     ï¿½necessï¿½io para saber qual ferramenta estï¿½sendo
     utilizada, jï¿½que este arquivo faz parte de quatro
  ...
  if (session_is_registered("cod_ferramenta_m") && (isset($cod_ferramenta)))
  {
	...
	$ultima_pasta_criada = ""; //limpa a variï¿½el uma vez que o usuï¿½io mudou de ferramenta      
    }
  }
...
/* Inicio da Paina do Formador ************************/
    ...
    if ($acao=="movertopico")
    {
      ...
      if (NaoExisteTop($sock, $tabela, $cod_topico_raiz, $cod_topico, $cod_usuario)) 
      {
        ...
        {
          /* 2 - Vocï¿½nï¿½ pode mover uma pasta para ela mesma ou para uma subpasta dela. */
	...
	else
	{
	  ...
	  if (!$existe)
	  {
	    ...
	    // se tiver itens selecionados, move-os tambï¿½
	    if (strlen($cod_itens))
	    {
	  ...
	  {
	    /* 71- Nï¿½ foi possï¿½el mover a pasta, pois jï¿½existe uma pasta com mesmo nome no diretï¿½io destino. */
...
\n");
    ...
    if ($acao=="novotopico")
    {
      if (NaoExisteTop($sock, $tabela, $cod_topico_raiz, $novo_nome, $cod_usuario))
      {
        ...
        $ultima_pasta_criada = $novo_nome; // guardo o nome da pasta criada para nï¿½ duplica-la caso o usuario voltar pelo browser
      }
      else
      {
        if ($ultima_pasta_criada != $novo_nome)
...
*/
    ...
    echo("{\n");
    /* 71 - Vocï¿½tem certeza de que deseja apagar esta pasta? */
    /* 72 - (Todos os materiais contidos nessa pasta serï¿½ movidos para a lixeira) */
    ...
    if ($cod_ferramenta==3)
    {
      ...
      /* 117 - (as atividades serï¿½ movidas para a lixeira e se houver alguma avaliaï¿½o relacionada, as avaliaï¿½es tambï¿½ serï¿½ movidas para a lixeira DAS AVALIAï¿½ES) */
      echo("    return(confirm(\"".RetornaFraseDaLista($lista_frases,6)."\\n".RetornaFraseDaLista($lista_frases,117)."\"))\n");
      echo("  else\n");
      /* 6 - Vocï¿½tem certeza de que deseja apagar esta atividade? */
      /* 101 - (a atividade serï¿½movida para a lixeira e se houver alguma avaliaï¿½o relacionada, a avaliaï¿½o tambï¿½ serï¿½movida para a lixeira DAS AVALIAï¿½ES) */
    ...
    */
    ...
    $cod_pagina=1;
    if(($cod_ferramenta==3)&&($AcessoAvaliacao)&&(EFormador($sock,$cod_curso,$cod_usuario)))/*verificaï¿½o se aparecerï¿½ajuda de avaliaï¿½es*/
    $cod_pagina=6;
    ...
    {
      echo("  <tr class=text>\n");
      /* 15 - 3: Nï¿½ hï¿½nenhuma atividade
              4: Nï¿½ hï¿½nenhum material de apoio
              5: Nï¿½ hï¿½nenhuma leitura
              7: Nï¿½ hï¿½nenhuma parada obrigatï¿½ia
       */
...
//    echo("        <a class=text href=editar_material.php onClick=\"this.search='?".RetornaSessionID()."&origem=material&cod_curso=".$cod_curso."&cod_topico=".$cod_topico_raiz."&origem=material&cod_item='+js_cod_item\">".RetornaFraseDaLista($lista_frases_geral,9)."</a><br>\n");
    ...
    echo("  <tr>\n");
    echo("    <td>\n");
    /* 22 - Digite o novo tï¿½ulo do item: */
...
/*    echo("  <input type=hidden name=tipo_comp value=\"\">\n");*/
    ...
    echo("    <td>\n");
    echo("      <font class=text>\n");
    /* 23 - Digite o nmero da posiï¿½o em */
...
/* Inï¿½io da Pï¿½ina do Aluno e do Visitante ************/
...
//              echo ("<font size=8>TERCEIRO</font>\n");
            ...
            else
            {
              ...
              $nao_exibir=true; /* Alunos nï¿½ veï¿½ itens sendo editados */
            }
          }
          else
          {
...
//              echo ("<font size=8>QUARTO</font>\n");
          ...
          {
            if ($nao_exibir) /* Itens em ediï¿½o nï¿½ sï¿½ visto por alunos */
            {
              $nao_exibir=false;
            }
            else

...
function addImage(rte) {
	...
	imagePath = prompt('Entre com a URL da imagen:', 'http://');				
...
function writeRichTextOnJS_gabarito(rte, html, width, height, buttons, readOnly, id, nobuttons)
{
    ...
    if (isRichText) {
        ...
        if(!nobuttons){
           ...
           formTmp+=('  <td>');
           formTmp+=('      <ul class="rteBtConfirm">');
                      formTmp+=('        <li><span onClick="EdicaoTexto_gabarito(\''+cod+'\', \''+id+'\', \'canc\');" id="CancelaEdita" name="cancelar">Cancelar</span></li>');
                         formTmp+=('        <li><span onClick="EdicaoTexto_gabarito(\''+cod+'\', \''+id+'\', \'ok\');" id="OkEdita">OK</span></li>');
           formTmp+=('      </ul>');
           formTmp+=('    </td>');
           formTmp+=(' </tr>');
           formTmp+=('</table');

...
//				} else {
//					/*
//					   Colocar no caso de Exercicio em grupo. ï¿½ quase igual o cï¿½digo acima cima porï¿½m deve-se obter o codigo do grupo
...
favaliado 	Fórum de Discussão 	27/10/2009 	27/10/2019ent.frmAvaliacao.cod_avaliacao.value = id;\n");
  ...
  if ($usr_formador) 
  {
	...
	echo("      {\n");
	// 117 - Deseja realmente apagar a avaliaï¿½ï¿½oo selecionada? (Esta atividade deixaria de ser avaliada e a avaliaï¿½ï¿½oo seria movida para a lixeira DAS AVALIAï¿½ï¿½ES)*/
...
$cor_link2[$tela_avaliacao] = "</font>";
  ...
  if (count($lista_avaliacoes) > 0) 
  {
	foreach ($lista_avaliacoes as $cod => $linha) 
		...
		/*Caso seja uma Avaliaï¿½ï¿½o externa*/
		else
	...
	switch ($tela_avaliacao) {
		case 'P' :
			// 177 - Nao existem avaliaï¿½ï¿½es passadas
			$nao_existe = RetornaFraseDaLista($lista_frases, 177);
			break;
		case 'A' :
			// 38 - Nï¿½o existem avaliaï¿½ï¿½es atuais!
			$nao_existe = RetornaFraseDaLista($lista_frases, 38);
			break;
		case 'F' :
			// 178 - Nao existem avaliaï¿½ï¿½es futuras

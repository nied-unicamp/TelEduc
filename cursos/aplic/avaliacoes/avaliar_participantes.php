...
/*==========================================================
  ...
  if ($usr_formador)
  {
    // para o formador, esta �a tela em que ele avalia os alunos
  ...
  {
    // para o aluno, esta �a tela com os histricos de desempenho
    ...
    if ($num_grupos > 0)
    {
      ...
      if ($usr_formador)
      {
        if($avaliacao_participante)
        {
          ...
          echo("                    <td align=center width=21%>".RetornaFraseDaLista($lista_frases,62)."</td>\n");
        }
        // 63 - Compartilhamento
      ...
      foreach ($lista_grupos as $cod_grupo => $nome)
      {
        ...
        // retorna o codigo de um aluno que tem mais notas no grupo (caso aconteca) para garantir que retorne todas as avalia��es.
          ...
          else
          {
            ...
            if ($usr_formador)
            {
              ...
              else if($avaliacao_participante)
              {
               echo("                   <td align=center>");
                // 66 - Reavaliar
                echo(RetornaFraseDaLista($lista_frases,66)."</td>\n");
              }
              if (!isset($SalvarEmArquivo))
  ...
  else
  {
    ...
    if (count($lista_users) > 0)
    {
      ...
      if ($usr_formador)
      {
        if($avaliacao_participante)
        /* 62 - Avaliar/Reavaliar */
        echo("                    <td align=center width=21%><b> ".RetornaFraseDaLista($lista_frases,62)."</td>\n");
        /* 63 - Compartilhamento */
      ...
      foreach($lista_users as $cod => $nome)
      {
        ...
        echo("                    <td>");
        if (!isset($SalvarEmArquivo))
          echo("<span class=\"link\" onClick=return(AbrePerfil(".$cod.")); class=text>".$nome."</span></td>\n");
        else
          echo($nome."</td>\n");
    ...
    if ($usr_formador)
    {
      ...
      {
        // Tabela com a lista de formadores do curso, com suas respectivas notas na avalia�o realizada
        echo ("                <tr class=\"head\">\n");
        // 156 - Formadores
        ...
        if ($usr_formador)
        {
          if($avaliacao_participante)
          // 62 - Avaliar/Reavaliar
          echo("                    <td align=center width=21%><b> ".RetornaFraseDaLista($lista_frases,62)."</b></td>\n");
          // 63 - Compartilhamento

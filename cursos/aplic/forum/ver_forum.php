...
/*==========================================================
  ...
  /* Obtém a data e hora do penltimo acesso para comparar com as datas das   */
  /* mensagens e destacar a que são mais recentes.                            */
  $penult_acesso = PenultimoAcesso($sock, $cod_usuario, "");

  /* Se a data de alteração não estiver setada então entrou pela primeira vez */
  /* As páginas que modificam a base de dados (compor, responder, apagar e    */
  /* excluir mensagens) não repassam esse valor, pois assumem que houve       */
  /* alterações.                                                              */
  if ((!isset($data_msg_alt)) || (MensagensAlteradas($sock, $cod_forum, $data_msg_alt)))
  {
    /* Obtém a data atual para posterior comparação. Esta data é comparada com */
    /* as das mensagens. Se alguma mensagem possuir data superior a            */
    /* $data_msg_alt então lista novamente as mensagens, do contrário utiliza  */
    /* as mensagens armazenadas na sessão.                                     */
    $data_msg_alt = time();
    ...
    /* Se a ordenação for por árvore então chama a funãoo RetornaMensagens, que */
    ...
    {
      /* Se a ordenação for por data, emissor ou título da mensagem, chama a      */
      /* RetornaMensagensOrdenadas, que retorna as mensagens de acordo com a      */
      /* ordenação especificada porém sem estruturação (identação).               */
      list ($total_mensagens, $array_mensagens, $sin_pag) =
  ...
  /* Se não foram incluídas/alteradas mensagens após a data especificada em     */
  /* $data_msg_alt, utiliza as mensagens armazenadas.  RECOMENTAR               */
  {
   ...
   /* página então calcula o total de páginas. Do contráio, define o nmero de  */
   /* páginas para 1.                                                            */
  ...
  /* Se a página atual não estiver setada então, por padrão, atribui-lhe o valor 1. */
  /* Se estiver setada, verifica se a página é maior que o total de páginas, se for */
  /* atribui o valor de $total_pag à $pag_atual.                                    */

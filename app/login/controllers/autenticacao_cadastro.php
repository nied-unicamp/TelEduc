...
/*class AutenticacaoCadastro extends Controller{
	...
	if(!isset($login)){
		...
		if ($cod_usuario == 0)
		{
			...
			header("Location:".$view_login."autenticaчуo_cadastro.php?cod_curso=".$cod_curso."&acao=erroAutenticacao&atualizacao=false");
			exit;
		}
		...
		if(!AcessoPHP::VerificaConfirmacaoEmail($login)) {
			AcessoSQL::Desconectar($sock);
			header("Location:".$view_login."autenticaчуo_cadastro.php?cod_curso=".$cod_curso."&acao=erroConfirmacao&atualizacao=false");
			exit;
		}
		...
		/* Se a autenticaчуo for para inscricao, manda para tela de inscricao */
		...
		*/
		...
		{
			/* Verifica se quem esta fazendo login щ o administrador do ambiente, admtele */

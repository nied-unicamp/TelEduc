<?php
  ...
  * permanently store and retrieve Permissao object instances. 
  */
...
class PermissaoDao {
    ...
    function load(&$conn, &$valueObject) {
          ...
          if (!$valueObject->getCod_tipo_permissao()) {
               //print "Can not select without Primary-Key!";
               return false;
          }

          $sql = "SELECT * FROM Permissao WHERE (Papel_cod_papel = ".$valueObject->getCod_papel()." AND ";
    ...
    function searchMatching(&$conn, &$valueObject) {
          ...
          if ($valueObject->getCod_papel() != 0) {
              ...
              $sql = $sql."AND Papel_cod_papel = ".$valueObject->getCod_papel()." ";
          }
          ...
          if ($valueObject->getCod_tipo_permissao() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND Tipo_permissao_cod_tipo = ".$valueObject->getCod_tipo_permissao()." ";
          }


          $sql = $sql."ORDER BY Tipo_permissao_cod_tipo ASC ";
    ...
    function singleQuery(&$conn, &$sql, &$valueObject) {
          ...
          if ($row = $conn->nextRow($result)) {
                   ...
                   $valueObject->setCod_papel($row[0]); 
                   $valueObject->setCod_ferramenta($row[1]); 
                   $valueObject->setCod_tipo_permissao($row[2]); 
          } else {
    ...
    function listQuery(&$conn, &$sql) {
          ...
          while ($row = $conn->nextRow($result)) {
               ...
               $temp->setCod_papel($row[0]); 
               $temp->setCod_ferramenta($row[1]); 
               $temp->setCod_tipo_permissao($row[2]); 

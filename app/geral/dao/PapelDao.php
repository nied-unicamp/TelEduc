...
require_once '../../../lib/Conexao.php';
  ...
  * permanently store and retrieve Papel object instances. 
  */
...
class PapelDao {
    ...
    function searchMatching($conn, $valueObject) {
          ...
          if ($valueObject->getNome_papel() != "") {
              if ($first) { $first = false; }
              $sql = $sql."AND nome_papel LIKE '".$valueObject->getNome_papel()."%' ";
          }
    ...
    function singleQuery($conn, $sql, $valueObject) {
          ...
          if ($row = $conn->nextRow($result)) {
                   ...
                   $valueObject->setNome_papel($row[1]); 
          } else {

...
function xajaxCompressFile($sFile)
{
	...
	for($i=0; $i<$iLen; ++$i)
	{
		...
		$line = preg_replace_callback('/\/\/(.*)/',create_function('$matches','return \'\';'),$line);
		//strip leading and trailing whitespace
		$line = trim($line);
	
		//remove all whitespace with a single space
		//$line = preg_replace("/\s+/"," ",$line);
		$line = preg_replace_callback('/\s+/',create_function('$matches','return \' \';'),$line);
	
		//remove any whitespace that occurs after/before an operator
		//$line = preg_replace("/\s*([!\}\{;,&=\|\-\+\*\/\)\(:])\s*/","\\1",$line);
		$line = preg_replace_callback('/\s*([!\}\{;,&=\|\-\+\*\/\)\(:])\s*/',create_function('$matches','return $matches[1];'),$line);
	...
	//$sFile = preg_replace("/;\n/",";",$sFile);
	$sFile = preg_replace_callback('/;\n/',create_function('$matches','return \';\';'),$sFile);
